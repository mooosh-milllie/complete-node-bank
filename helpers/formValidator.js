...
const extLocalBankTransferSchema = yup.object({
  accountName: yup.string().required().min(3).max(250),
  amount: yup.number().required().test('not less than or equal to', 'Amount should be greater than zero',
  function(amount) {
    return amount > 0;
  }),
  ...
  bankName: yup.string().required().max(250),
  description: yup.string().required().max(255),
  routingNumber: yup.number().required().test('digits equal nine', 'Routing number should be 9 digits',
  function(rn) {
    return rn.toString().length === 9;
  }),
})
const webhookLocalBankTransferSchema = yup.object({
  senderName: yup.string().required().min(3).max(250),
  amount: yup.number().required().test('not less than or equal to', 'Amount should be greater than zero',
  function(amount) {
    return amount > 0;
  }),
  senderAcct: yup.number().required().test('Greater than three digits', 'Account number must be greater than 3 digits',
  function(accountNumber) {
    return String(accountNumber).length > 3 && String(accountNumber).length < 21;
  }),
  senderBankName: yup.string().required().max(250),
  description: yup.string().required().max(255),
  receiverAcct: yup.number().required().test('digits equal nine', 'Account number should be 9 digits',
  function(acctNo) {
    return String(acctNo).length === 10;
  }),
  senderBankRN: yup.number().required().test('digits equal nine', 'Routing number should be 9 digits',
  function(rn) {
    return String(rn).length === 9;
  }),
  trxId: yup.string().required().min(10)
})
...
module.exports = {
  ...
  extLocalBankTransferSchema,
  webhookLocalBankTransferSchema
}