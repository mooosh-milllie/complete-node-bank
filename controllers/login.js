...
const {Login, CustomerLoginInfo, Customers, Accounts} = require('../models/index');
...
router.post('/verify-otp', async(req, res, next) => {
  ...
  try {
    ...
    await existingUser.createLogin_info({
...
router.post('/forgot-password/check-details', async(req, res, next) => {
  try {
    ...
    const currentAccount = registeredUser.get('account');

    if (currentAccount.pin !== validateInput.pin) return res.sendStatus(401);
