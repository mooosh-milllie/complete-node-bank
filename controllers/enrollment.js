...
const {Customers, Login, Accounts} = require('../models/index');
...
router.post('/check', async (req, res, next) => {
  ...
  try {
    ...
    const account = registeredUser.get('account');

    const pinToNum = Number(validateInput.pin);
    
    if (!registeredUser || account.pin !== pinToNum) return res.sendStatus(401);
...
router.post('/create-login/:token', enrollmentMiddleware, async(req, res, next) => {
  try {
    ...
    await user.createLogin({username: validateInput.username, password: hashedPassword, customer_id: user.id});
